...
const Work: React.FC<Props> = ({ parentContainer, order, last }) => {
    ...
    useEffect(() => {
        ...
        const start = `top ${0 + (-window.innerHeight * order) + (window.innerHeight / 2)}`
        const end = `top ${-20 + (-window.innerHeight * (order + 1)) + (window.innerHeight / 2)}`
        ...
        const imageMove = gsap.fromTo(imageRef.current, {
            translateY: "5%",
        }, {
            translateY: "-3%",
            duration: 12
    ...
    return (
        <div ref={containerRef} className="grid grid-cols-2 sm:grid-cols-1 md:grid-cols-2 md:h-full h-screen w-full absolute top-0">
            ...
            <div className="flex justify-center items-center sm:h-[70vh] sm:items-start md:items-center md:h-full py-24">
                <img ref={imageRef} alt="its random" src="https://picsum.photos/400/800" width="400" height="800" />
            </div>
        </div>
    )
};
